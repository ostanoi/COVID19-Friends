---
title: "Median Negative & Positive Affect in Daily Life During COVID-19 ~ Number of Close Friends * Number of Stressors"
...
```{r, message = F, warning = F}
ggplot(covid_ema, aes(NegativeMood)) +
...
```{r, message = F, warning = F}
ggplot(covid_ema, aes(PositiveMood)) +
  geom_density(fill = "deepskyblue2", color = "deepskyblue4", alpha = .8) + 
...
Show the distribution of the number of close friends variable.<br>
```{r, message = F, warning = F}
ggplot(df, aes(baseline_closest_deg_out_norm)) +
  geom_histogram(fill = "deepskyblue2", color = "deepskyblue4", binwidth = 2) + 
  labs(x = "Number of Close Friends in College Group",
       y = "Count", 
       title = "") +
...
Run regression analysis 1: median negative affect ~ number of close friends * number of COVID-19 stressors.<br>
...
Run regression analysis 2: median positive affect ~ number of close friends * number of COVID-19 stressors.<br> 
...
```{r}
load(sprintf("%s/Models/1. Median-NegAff_Out-Degree_Self-Stressors.RData", path))
load(sprintf("%s/Models/1. Median-PosAff_Out-Degree_Self-Stressors.RData", path))
tab_model(f1, f2, show.se = TRUE,
          show.r2 = TRUE, 
          dv.labels = c("Median Negative Affect",
                        "Median Positive Affect "),
          pred.labels = c("(Intercept)", 
                          "Number Friends Prior to COVID-19",
                          ...
                          "Physical Distance",
                          "EMA Completion Rate",
                          "Number Friends * Number of Stressors Interaction"))
```
<br>
...
Calculate the proportion of the posterior distribution that is lower than 0 for the main effect of the number of close friends on median negative affect.<br>
...
Calculate the proportion of the posterior distribution that is lower than 0 for the interaction effect between participants' number of close friends and their number of stressors on median negative affect.<br> 
...
Calculate the proportion of the posterior distribution that is higher than 0 for the main effect of the number of close friends on median positive affect.<br>
...
Calculate the proportion of the posterior distribution that is higher than 0 for the interaction effect between participants' number of close friends and their number of stressors on median positive affect.<br> 
...
# Define the number of posterior draws
...
g3 <- g2 +
  annotate(geom="text", x=-2.8, y=96, size = 4, color="#810F7C",
           label="b = 1.79, 95%CI = [0.31, 3.24], 99% post. distr. > 0") +
   annotate(geom="text", x=-2.6, y=89, size = 4, color="#2D004B",
           label="b = -0.35, 95%CI = [-1.57, 0.86], 29% post. distr. > 0") 
g3
```
<br>
...
g3 <- g2 +
  annotate(geom="text", x = -2.45, y = 8, size = 4, color="#810F7C",
           label="b = -1.84, 95%CI = [-3.43, -0.30], 99% post. distr. < 0") +
    annotate(geom="text", x = -2.55, y = 1, size = 4, color="#2D004B", 
           label="b = 0.04, 95%CI = [-1.30, 1.37], 48% post. distr. < 0")
g3
```
<br>
