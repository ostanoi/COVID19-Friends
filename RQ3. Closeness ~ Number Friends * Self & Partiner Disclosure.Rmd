---
title: "Closeness ~ Number of Friends * Self/Conversation Partner Disclosure"
...
```{r, message = F, warning = F}
# Extract for each participant their number of close friends, demographic information, and COVID-19 information
df <- covid_ema %>%
  group_by(pID) %>%
...
Run regression analysis 1: closeness ~ number of friends * self-disclosure (within component).<br>
...
#                      SalInt_Partner_Close_cw + baseline_college_year_c +
#                      baseline_gender_numeric_c + baseline_income_num_c +
#                      baseline_educ_parents_years_c + covid_housing_tension_c +
...
**RQ**: Do people feel closer to their interaction partners when the interactions partners disclose something personal? Is the relationship between conversation partner disclosure and closeness mediated by one's number of close friends?<br>

Run regression analysis 1: closeness ~ number of close friends * conversation partner disclosure (within component).<br>
...
```{r, message = F, warning = F, echo = F}
load(sprintf("%s/Models/3.Closeness_Out-Degree_Self-Disclose.RData", path))
load(sprintf("%s/Models/3.Closeness_Out-Degree_Partner-Disclose.RData", path))
```
...
tab_model(fit1, fit2, show.se = TRUE,
          ...
          pred.labels = c("(Intercept)", 
                          ...
                          "HH Tension",
                          "Physical Distance",
                          "Number Friends * Self Disclosure (WI) Interaction",
                          "Partner Disclosure (WI)",
                          "Number Friends * Partner Disclosure (WI) Interaction"))
```
...
Calculate the proportion of the posterior distribution that is higher than 0 for the main effect of number of close friends on closeness.<br>
...
Calculate the proportion of the posterior distribution that is lower than 0 for the interaction effect between participants' number of close friends and self-disclosures on closeness.<br> 
...
##### Model 2: Predictor Variable = Conversation Partner-Disclosure
<br>
...
Calculate the proportion of the posterior distribution that is higher than 0 for the main effect of conversation partner disclosures on closeness.<br> 
...
Calculate the proportion of the posterior distribution that is lower than 0 for the interaction effect between participants' number of close friends and conversation partner disclosures on closeness.<br> 
...
```{r, message = F, warning = F}
# Calculate mean number of close friends  +/-1.5SD
...
Test the hypothesis: does the slope representing the relationship between conversation partner disclosure and closeness differ from zero for 1) students with few friends at school (1.5 SD below mean), 2) students with many friends at school (1.5 SD above mean)?<br>
...
Are the two slopes different from each other?<br>

```{r, message = F, warning = F}
my_hyp_up <- hypothesis(fit2, "1*SalInt_Disc_Partner_cw +
                      -14.884*SalInt_Disc_Partner_cw:baseline_closest_deg_out_norm_c = 
                      1*SalInt_Disc_Partner_cw +
                      +14.884*SalInt_Disc_Partner_cw:baseline_closest_deg_out_norm_c")
p <- my_hyp_up$samples
df_hyp <- my_hyp_up$hypothesis[ ,2:5] %>% 
  mutate(Prop.Above.Zero = length(which(p$H1 > 0))/n) %>% 
...
ggplot(ef, aes(p_slope)) +
  geom_histogram(fill = "#810F7C", 
                 ...
                 binwidth = 1) + 
  labs(x = "Person-Level Effects of Self Disclosure (WI)",
       y = "Count") +
...
ggplot(ef, aes(p_slope)) +
  geom_histogram(fill = "#810F7C", 
                 color = "#2D004B", alpha = .6,
                 binwidth = 1) + 
  labs(x = "Person-Level Effects of Partner Disclosure (WI)",
       y = "Count") +
  scale_x_continuous(breaks = c(0,4,8,12,16,20,24)) +
  expand_limits(x=c(0, 24)) +
